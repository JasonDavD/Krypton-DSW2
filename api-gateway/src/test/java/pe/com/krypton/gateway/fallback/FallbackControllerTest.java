...
class FallbackControllerTest {
    ...
    @Test
    void catalogo_returns_503_with_message() {
        var resp = controller.catalogo();
        assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.SERVICE_UNAVAILABLE);
        assertThat(resp.getBody()).containsEntry("status", 503);
        assertThat((String) resp.getBody().get("message")).contains("catálogo");
    }

    @Test
    void pedidos_returns_503_with_message() {
        var resp = controller.pedidos();
        assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.SERVICE_UNAVAILABLE);
        assertThat(resp.getBody()).containsEntry("status", 503);
        assertThat((String) resp.getBody().get("message")).contains("pedidos");
    }
}
